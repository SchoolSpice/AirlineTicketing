...
class DB {
	...
	ResultSet searchFlights(String d, String a, Calendar c) throws Exception {
		Date date = c.getTime();
		PreparedStatement query =
				conn.prepareStatement("SELECT * FROM airlinedb.flights " +
				                      "WHERE flights.departurelocationid = '" +
									  d + "', flights.arrivallocationid = '" +
									  a + "', flights.departdate = '" + date.getYear() +
									  "-" + date.getMonth() + "-" + date.getDay() + "'");
		return query.executeQuery();
	} //end-searchFlights
    
    private void insertCustomer(final String FIRST_NAME,
                                final String LAST_NAME) throws Exception {
